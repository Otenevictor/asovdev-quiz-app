...
const resultContainer = document.querySelector(".result-container");
 ...
 // JavaScript to close the config popup
    ...
    const configPopup = document.getElementById('configPopup');
      // JavaScript to close the quiz popup
      const closeQuizBtn = document.getElementById('closeQuizBtn');
      const quizPopup = document.getElementById('quizPopup'); // Ensure this ID matches your quiz popup
  
      document.addEventListener("DOMContentLoaded", () => {
        const closeQuizBtn = document.getElementById('closeQuizBtn');
        const quizPopup = document.querySelector('.quiz-popup');
    
        if (closeQuizBtn && quizPopup) {
            closeQuizBtn.addEventListener('click', () => {
                quizPopup.classList.remove('active'); // Hide the popup
            });
        }
    });
    closeConfigPopup.addEventListener('click', () => {
        configPopup.classList.remove('active');
    });

 // begin quiz Configuration Popup
 openQuizBtn.addEventListener('click', () => {
     configPopup.classList.add('active');
 });

 openQuizBtn2.addEventListener('click', () => {
     configPopup.classList.add('active');
 });
